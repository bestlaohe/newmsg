    ...
    for (int size = initial_size; size <= final_size; size += step) {
     ...
     //  Paint_Clear(GREEN);
       ...
       // Delay_Ms(100);
        ...
        Paint_Drawicon(icon_x, icon_y, 3, &Font24_icon, BLACK, BLACK);//去掉残影
    }
   // Paint_Clear(BLACK);
...
int main(void)
{
    ...
    /*********************基本内容初始化******************************/
    SystemCoreClockUpdate();   // 48000000系统时钟刷新
...
Delay_Ms(100);
  ...
  //Paint_Drawicon(111, 104, 3,&Font24_Num,  BLACK, WHITE);   //  开机界面
